...
export default class HotelFilterAppHome extends LightningElement {
    ...
    handleTypeChange(event){
      ...
      if(!this.filters.selectedTypes){
        ...
        this.filters = {...this.filters,rooms:roomsT}

        if(event.target.checked){

            console.log('button checked')

            if(!this.filters.rooms.includes(event.target.dataset.id)){
            ...
            console.log('filter action started target id =', event.target.dataset.id)

            console.log(this.filters.rooms = this.filters.rooms)
          this.filters.rooms = this.filters.rooms.filter(element => element !== event.target.dataset.id);
