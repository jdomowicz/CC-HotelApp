...
export default class HotelFilterAppHome extends LightningElement {
    ...
    handleTypeChange(event){
        ...
        this.filters = {...this.filters,romtypes:this.roomTypePicklist}
        console.log(this.filters);

        this.filters.romtypes.forEach(element=>{

            if(element.label == event.target.dataset.id){

               // console.log({element:element.value,value:event.target.checked})
                this.filters.romtypes[element.label] = [...this.filters.romtypes,{label:element.value,value:event.target.checked}]
            }
