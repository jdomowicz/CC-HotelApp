 ...
 gethotelListDropdown() {
     ...
     getRoomListDropdown() {
        ...
        roomList().then((result) => {
            ...
            result.forEach(element => {
                this.roomList = [...this.hotelList, { "label": element.Name, "value": element.Id, "Id": element.Id,"Rooms_Available__c":element.Rooms_Available__c,"Rating__c":element.Rating__c }];
            });
        console.log(this.hotelList);
        }).catch((error) => {
            console.error(error);
